 ...
 * @author Rodrigo
 */
public class Tela_Fazer_Atendimento_DAO {
    private Connection instancia;
    public Tela_Fazer_Atendimento_DAO(){
    ...
    public String[] Retorna_Noticias(){
        ...
        String sql="SELECT agenda.pago, agenda.CODAGENDAMENTO, agenda.DAT as datas, agenda.HORA,"
    ...
    public String[] Retorna_Noticias(String paciente, String medico){
        ...
        String sql="SELECT agenda.pago, agenda.CODAGENDAMENTO, agenda.DAT as data, agenda.HORA, funcionario.nome as nomefun,"
                ...
                + "AND AGENDA.CODAGENDAMENTO NOT IN(SELECT IDCONSULTA FROM RECEITAS);";
        try {
            instancia.setAutoCommit(true);
            PreparedStatement stm= instancia.prepareStatement(sql);
    ...
    public boolean Insert_Receita(int idconsulta, int paciente, int idmedico, String receita){
        String sql="INSERT INTO receitas(IDCONSULTA, IDPACIENTE, IDMEDICO, RECEITA) VALUES"
                +"(?, ?, ?, ?)";
        try {
            instancia.setAutoCommit(true);
            PreparedStatement stm= instancia.prepareStatement(sql);
            ...
            stm.setInt(3, idmedico);
            stm.setString(4, receita);
            stm.execute();
            ResultSet results=stm.getResultSet();
            stm.close();
            return true;
